...
def quick_sort(arr):
    ...
    # 基准值通过random随机选取
    # 入参: 待排序数组, 数组开始索引 0, 数组结束索引 len(basic04_array)-1
    ...
    def quick_sort_two_way(arr, low, upper):
        # 小数组排序i可以用插入或选择排序
        ...
        # 将基准值的索引从下边界调换到索引分割点
        swap(arr, low, j)
        quick_sort_two_way(arr, low, j - 1)
        quick_sort_two_way(arr, j + 1, upper)
        return arr

    return quick_sort_two_way(arr, 0, len(arr) - 1)
